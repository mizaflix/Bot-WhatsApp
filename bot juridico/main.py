...
def bot():
    ...
    if msg.lower() in ["início", "recomeçar"]:
        users[sender] = {'step': 0}
        msg.body("✅ Conversa reiniciada. Digite qualquer coisa para começar.")
        return str(resp)
    ...
    if step == 0:
        ...
        msg.body(f"Olá! Qual a área jurídica do seu interesse?\n{area_list}\n\nDigite exatamente como está.")
        ...
        if msg in areas:
            ...
            msg.body(f"Entendi! Sobre o que se trata especificamente?\n{texto}\n\nResponda com o número da opção.")
        else:
            msg.body("Área inválida. Por favor, digite uma das opções corretamente.")
        ...
        try:
            ...
            if 0 <= idx < len(user['opcoes_descricao']):
                ...
                user['step'] = 3
                users[sender] = user
                msg.body("Você tem como comprovar isso? (Sim ou Não)")
            else:
                raise ValueError
        except:
            msg.body("Opção inválida. Responda com o número da opção desejada.")
        ...
        if msg.lower() in ["sim", "não"]:
            ...
            user['step'] = 4
            users[sender] = user
            msg.body("Por meio de quê você pode comprovar? (ex: holerite, testemunha, laudo médico...)")
        else:
        ...
        user['step'] = 5
        users[sender] = user
        msg.body("Tudo isso é verídico? (Sim ou Não)")
        ...
        if msg.lower() in ["sim", "não"]:
            ...
            user['step'] = 6
            users[sender] = user
            msg.body("Baseado em seu caso, você tem direito a valores entre R$5.000 a R$25.000. Deseja prosseguir com a geração do contrato? (Sim ou Não)")
        else:
            msg.body("Responda com 'Sim' ou 'Não'.")
        ...
        if msg.lower() == "sim":
            ...
            user['step'] = 7
            users[sender] = user
            msg.body(f"✅ Contrato gerado com sucesso! Acesse aqui:\n{url}")
            msg.body(f"Entre em contato com o advogado para dar continuidade: {ADVOGADO_NUMERO}")
        else:
            msg.body("Ok, processo encerrado. Se quiser reiniciar, digite 'início'.")
            users[sender] = {'step': 0}

    else:
        msg.body("Digite 'início' para começar uma nova conversa.")
        users[sender] = {'step': 0}
